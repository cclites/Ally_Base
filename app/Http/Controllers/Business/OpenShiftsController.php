        ...
        if( request()->filled( 'json' ) ){
            ...
            $results = Schedule::forRequestedBusinesses()
                ->with([ 'client', 'schedule_requests' => function( $q ){
                    ...
                    return $q->whereIn( 'status', [ OpenShiftStatus::REQUEST_PENDING, OpenShiftStatus::REQUEST_UNINTERESTED ] );
                }])
                ->ordered()
            ...
            $schedules = $results->map( function( Schedule $schedule ) {
                ...
                return [
                    ...
                    'requests_count'    => $schedule->schedule_requests->filter( function( $r ){ return in_array( $r->status, [ OpenShiftStatus::REQUEST_PENDING, OpenShiftStatus::REQUEST_UNINTERESTED ]); })->count()
                ];
            });
