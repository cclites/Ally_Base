...
class ClientCaregiver implements Responsable
{
    ...
    public function toResponse($request)
    {
        ...
        if ($caregiver->pivot) {
            $caregiver->pivot->ally_hourly_fee = number_format(AllyFeeCalculator::getFee($client, $client->defaultPaymentMethod,
                $caregiver->pivot->caregiver_hourly_rate + $caregiver->pivot->provider_hourly_fee), 2);
            $caregiver->pivot->ally_daily_fee = number_format(AllyFeeCalculator::getFee($client, $client->defaultPaymentMethod,
