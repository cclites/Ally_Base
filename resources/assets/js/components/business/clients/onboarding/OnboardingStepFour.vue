<template>
    <div>
        ...
        <b-row>
            <b-col>
                <p>This Agreement, effective this {{ moment.format('Mo') }} day of {{ moment.format('MMMM') }}, {{ moment.format('YYYY') }} (the “Effective Date”), is between
                    {{ onboarding.client.business.name }} LLC d/b/a Granny NANNIES, an independently owned and operated franchise location,
                    which is a licensed nurse registry under Florida Statutes §400.506 (hereinafter referred to as “Nurse
                    Registry”), located at 4045-A Tamiami Trail, Port Charlotte, FL 33952 and {{ onboarding.client.business.name }}
                    located at {{ onboarding.client.business.address1 }} City/State {{ onboarding.client.business.city }}/{{ onboarding.client.business.state }} Zip: {{ onboarding.client.business.zip }}
                ...
                <ol>
                    ...
                    <li>The fees that Client will pay a Nanny for services that the Nanny provides for Client are separately
                        ...
                        Client agrees to pay Nurse Registry a referral fee of $ <b-form-input v-model="form.referral_fee" type="number" class="w-25"></b-form-input> per hour for each hour of service that
                    ...
                    <li>Either party hereto may terminate this Agreement by providing the other party with written notice at
                        least <b-form-input class="w-25" v-model="form.termination_notice"></b-form-input> prior to the date of termination.
                    </li>
                    ...
                    <li>Client understands that Nurse Registry, as required by applicable law, has access to Registered
                        ...
                        referral at Client’s request. The RNs generally charge a fixed fee for an assessment of $ <b-form-input type="number" v-model="form.per_visit_assessment_fee" class="w-25"></b-form-input>
                        per visit, although Client and a referred RN always remain free to negotiate a different fee. Nurse
                        Registry’s referral fee for referring an RN is $ <b-form-input type="number" v-model="form.per_visit_referral_fee" class="w-25"></b-form-input> per visit. The RN’s fee is payable by
                ...
                <div>
                    ...
                    <b-form-group label="Executed By">
                        <b-form-radio-group v-model="form.executed_by" name="radioSubComponent">
                            <b-form-radio value="client">Client</b-form-radio>
                            <b-form-radio value="poa">POA</b-form-radio>
                            <b-form-radio value="guardian">Guardian</b-form-radio>
                ...
                <b-row>
                    <b-col>
                        ...
                        <b-form-group label="Check all that apply">
                            <b-form-checkbox-group v-model="form.payment_options">
                                <b-form-checkbox value="1">Option 1: Check. Client will pay Nanny and Nurse Registry their respective fees by separate check to each.</b-form-checkbox>
                                <b-form-checkbox value="2">Option 2: Payment Processing Firm.  Client, or a third-party payor that Client designates (e.g., an insurance company), will pay Nanny and Nurse Registry their respective fees through a payment processing firm that Client will separately engage, which will function as an agent of Client.
  ...
  export default {
    ...
    data() {
      return {
          ...
          form: new Form({
              client_id: this.onboardingData.client_id,
              referral_fee: 0,
              per_visit_referral_fee: 0,
              per_visit_assessment_fee: 0,
              termination_notice: '',
              executed_by: '',
              payment_options: [],
              signature_one: '',
